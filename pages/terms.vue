<template>
  <main>
    <b-container fluid="md">
      <section>
        <b-row>
          <b-col>
            <h1>Terms of Care</h1>
          </b-col>
        </b-row>
        <b-row class="mt-6 mb-6">
          <b-col lg="5">
            <p>Dear Citizens, Companies, Organizations and Governments. By using a token (ie. downloading, sharing, implementation in a project) you agree to the following terms and conditions.</p>
          </b-col>
          <b-col lg="4" class="d-none d-lg-block">
            <ol class="list-unstyled paragraph text-uppercase f700">
              <li><a href="#term-1">1 Taking care</a></li>
              <li><a href="#term-2">2 No harm</a></li>
              <li><a href="#term-3">3 Discourse and dignity</a></li>
              <li><a href="#term-4">4 Inclusion</a></li>
              <li><a href="#term-5">5 Your responsibility</a></li>
              <li><a href="#term-6">6 Re-mix</a></li>
              <li><a href="#term-7">7 Commercial projects</a></li>
              <li><a href="#term-8">8 Include reference</a></li>
              <li><a href="#term-9">9 Share-alike</a></li>
            </ol>
          </b-col>
          <b-col cols="10" lg="2" class="mt-6 mt-lg-0">
            <p>In short: use the Tokens to make our planet better, not worse.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-1">
        <div class="section-divider" />
        <b-row class="bg-lighter">
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">1</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Taking care</h2>
          </b-col>
        </b-row>
        <b-row class="bg-lighter pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">Your use is taking care and strives for the common good.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-2">
        <div class="section-divider bg-lighter" />
        <b-row>
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">2</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">No harm</h2>
          </b-col>
        </b-row>
        <b-row class="pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">Your use does not harm people, animals or the environment.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-3">
        <div class="section-divider" />
        <b-row class="bg-lighter">
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">3</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Discourse and dignity</h2>
          </b-col>
        </b-row>
        <b-row class="bg-lighter pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">Your use respects civil discourse and human dignity.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-4">
        <div class="section-divider bg-lighter" />
        <b-row>
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">4</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Inclusion</h2>
          </b-col>
        </b-row>
        <b-row class="pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">Your use is as inclusive as possible and does not discriminate against people of different races, genders, sexual orientations and/or religions.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-5">
        <div class="section-divider" />
        <b-row class="bg-lighter">
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">5</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Your responsibility</h2>
          </b-col>
        </b-row>
        <b-row class="bg-lighter pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">You take sole responsibility for your project and resulting actions.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-6">
        <div class="section-divider bg-lighter" />
        <b-row>
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">6</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Re-mix</h2>
          </b-col>
        </b-row>
        <b-row class="pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">You are encouraged to modify, re-mix, adapt or change the Tokens to better suit your purpose.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-7">
        <div class="section-divider" />
        <b-row class="bg-lighter">
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">7</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Commercial projects</h2>
          </b-col>
        </b-row>
        <b-row class="bg-lighter pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">Commercial projects are explicitly allowed.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-8">
        <div class="section-divider bg-lighter" />
        <b-row>
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">8</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Include reference</h2>
          </b-col>
        </b-row>
        <b-row class="pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">When using a Token, you must include an adequate reference to this project.</p>
          </b-col>
        </b-row>
      </section>
      <section id="term-9">
        <div class="section-divider" />
        <b-row class="bg-lighter">
          <b-col cols="2" lg="1" offset-lg="1" class="text-lg-right">
            <span class="h1 pl-2 pl-lg-0">9</span>
          </b-col>
          <b-col cols="10" lg="5" class="text-right text-lg-left">
            <h2 class="h3 mt-3 mb-0 pr-2 pr-lg-0">Share-alike</h2>
          </b-col>
        </b-row>
        <b-row class="bg-lighter pt-3">
          <b-col cols="10" lg="5" offset-lg="2">
            <p class="text-smaller mb-0 mt-lg-n3 pl-2 pl-lg-0">When distributing Tokens, you impose the same conditions for their use.</p>
          </b-col>
        </b-row>
      </section>
      <section>
        <div class="section-divider bg-lighter" />
      </section>
      <section class="mt-6 mb-8">
        <b-row>
          <b-col lg="5" offset-lg="2">
            <p class="text-smallest">These terms have partly been inspired by the “Contract for the Web – A global plan of action to make our online world safe and empowering for everyone”. More information at <a href="https://contractfortheweb.org" target="_blank" rel="nofollow">contractfortheweb.org</a>.</p>
          </b-col>
        </b-row>
      </section>
    </b-container>
  </main>
</template>

<script>
export default {
  head: {
    title: 'TOKENS FOR CLIMATE CARE | Terms of Care'
  }
}
</script>
