          ...
          <b-row class="mt-4">
            <b-col>
              <p class="f700 text-smaller mb-0">
                <a :href="`${baseUrl}/archive/download?id=${token.id}&format=png`"><span class="material-icons mr-1">arrow_downward</span>Download PNG</a>
              </p>
            </b-col>
...
export default {
  ...
  methods: {
    ...
    downloadToken () {
      this.apiError = ''
      fetch(`${this.url}/svg?id=${this.$route.query.id}`, {
        method: 'GET',
        headers: {
